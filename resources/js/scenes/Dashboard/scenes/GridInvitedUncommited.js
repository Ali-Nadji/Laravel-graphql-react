...
class GridInvitedUncommited extends Component {
        constructor(props) {
            super(props);
            const invitedUnCommited = [];
            props.jmaker.map(function(item) {
                if(item.state === 'JMAKER_STATE_INVITED' || item.missions_ct === 0){
                    invitedUnCommited.push(item)
                }
            });
            this.state = {
                columnDefs: props.column,
                rowData: invitedUnCommited,
                domLayout: "autoHeight",
                getRowNodeId: function(data) {
                    return data.uuid
                }
            }
            this.props.setRowData(invitedUnCommited.length)
        }
            ...
            <div style={{ backgroundColor:'#f4f8f9' }}>
                ...
                >
                    <AgGridReact
                        ...
                        overlayNoRowsTemplate={"<div><img class='img-grid' src='/_partnerV2/images/invitation_waiting.svg'><p class='label-img'>"+t("no invitations")+"</p></div>"}
...
export default GridInvitedUncommited;
